#!/usr/bin/env python3
"""
Re-grade all MCP servers in leaderboard.html with agent-friend v0.69.0.
...
# Manual server-ID -> schema file mapping (from v0.65.0 script + additions)
MANUAL_MAP = {
...
def update_detail_row(content, server_id, new_data):
    ...
    def replace_detail(m):
        ...
        # Replace breakdown bars: 3 pairs of bar-fill + score
        bars = re.findall(
...
def main():
    # Find all server IDs in the leaderboard
    ...
    if changes:
        print(f"\nChanges:")
        for server_id, old, new, grade in sorted(changes, key=lambda x: x[1] - x[2] if x[1] else 0, reverse=True):
