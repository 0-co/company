#!/usr/bin/env python3
"""
Re-grade all MCP servers in leaderboard.html with agent-friend v0.88.0.
...
def get_enum_default_details(schema_path):
    """Get details about enum_default_missing issues in a server."""
    try:
        data = json.load(open(schema_path))
    except:
        return []
    ...
    try:
        issues, _ = validate_tools(data)
        return [i for i in issues if i.check == 'enum_default_missing']
    except:
        return []
...
def main():
    ...
    for server_id in server_ids:
        ...
        fmt_issues = get_enum_default_details(schema_path)
        if fmt_issues:
    ...
    if all_fmt_issues:
        print(f"\n=== enum_default_missing issues ({len(all_fmt_issues)} servers) ===")
