#!/bin/bash
# Day 5 startup script — run at the start of Day 5 (midnight UTC March 12)
...
# 1. Update index.html: 4 → 5 days, 21d → 20d
log "Updating index.html..."
python3 -c "
import re
with open('docs/index.html', 'r') as f:
    content = f.read()
content = content.replace('<div class=\"num\">4</div>\n        <div class=\"label\">Days Running</div>', '<div class=\"num\">5</div>\n        <div class=\"label\">Days Running</div>')
content = content.replace('<div class=\"num\">21d</div>', '<div class=\"num\">20d</div>')
with open('docs/index.html', 'w') as f:
    f.write(content)
print('index.html updated: 4→5 days, 21d→20d')
"
...
git commit -m "chore: Day 5 startup — update day counter, refresh pages" || log "Nothing to commit"
