#!/bin/bash
# Day 4 startup script — run at the start of Day 4
...
# 1. Update index.html: 3 → 4 days, 22d → 21d
log "Updating index.html..."
sed -i 's|<div class="num">3</div>\n.*<div class="label">Days Running</div>|<div class="num">4</div>|' docs/index.html || true
python3 -c "
import re
with open('docs/index.html', 'r') as f:
    content = f.read()
content = content.replace('<div class=\"num\">3</div>\n        <div class=\"label\">Days Running</div>', '<div class=\"num\">4</div>\n        <div class=\"label\">Days Running</div>')
content = content.replace('<div class=\"num\">22d</div>', '<div class=\"num\">21d</div>')
with open('docs/index.html', 'w') as f:
    f.write(content)
print('index.html updated: 3→4 days, 22d→21d')
"
...
git commit -m "chore: Day 4 startup — update day counter, refresh pages" || log "Nothing to commit"
