...
The result: 198 servers graded, average score 68.4, with the official Notion server at 19.8 (F) and Grafana at 21.9 (F).
...
Of the 198 servers graded, a significant fraction use hyphens (`resolve-library-id`), camelCase (`getDocumentContent`), or names that are nouns without verbs (`page_search`, `document`). None of these are wrong in the sense of failing JSON schema validation. All of them are wrong in the sense of making the LLM work harder.
...
The most common failure mode in the 198-server dataset: descriptions used as instruction manuals.
...
Of the 198 servers graded, several have parameters with no description at all. Some have parameters with just a type annotation. A few expose complex objects with nested fields and no documentation at any level.
...
Current situation: no guidance exists. The result is servers like GitHub MCP at 20,444 total tokens — large enough that loading it alongside two other servers could consume 40,000 tokens before the first user message.
...
Right now, that vocabulary doesn't exist in the spec. We've been building it from the outside: 198 servers, 3,971 tools, 511,518 tokens of data.
