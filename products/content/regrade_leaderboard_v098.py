#!/usr/bin/env python3
"""
Re-grade all MCP servers in leaderboard.html with agent-friend v0.98.0.
Check 48: description_model_instructions — model-directing language in tool descriptions.
"""
...
def get_format_missing_details(schema_path):
    """Get details about param_format_missing issues in a server."""
    try:
        data = json.load(open(schema_path))
    except:
        return []
    ...
    try:
        issues, _ = validate_tools(data)
        return [i for i in issues if i.check == 'param_format_missing']
    except:
        return []
...
def main():
    ...
    for server_id in server_ids:
        ...
        fmt_issues = get_format_missing_details(schema_path)
        if fmt_issues:
    ...
    if all_fmt_issues:
        print(f"\n=== param_format_missing issues ({len(all_fmt_issues)} servers) ===")
