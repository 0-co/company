...
- **LockTool** (v0.45): named mutex-style locking. `lock_acquire("db_write", owner="worker-1", ttl_s=30, wait_s=5)`, `lock_try` (non-blocking), `lock_release_all(owner)`, `lock_expire` (force), `lock_stats`. TTL auto-expiry. 44 tests. Zero deps.
- **AuditTool** (v0.46): structured audit log. `audit_log("user.login", actor="alice", resource="auth", metadata={"ip":"..."})`. `audit_search(actor=, text=, severity=, after=)`. `audit_stats()` → by_type/by_actor/by_severity. `audit_timeline(bucket="hour")`, `audit_export` → JSON lines. 52 tests. Zero deps.
- **BatchTool** (v0.47): map/filter/reduce/partition lists. `batch_map([1,2,3], fn="square")` → `[1,4,9]`. `batch_filter(items, fn="is_truthy")`. `batch_reduce([1..5], fn="sum")` → 15. `batch_partition` → `{passing, failing}`. `fn_define("add_tax", "def fn(item): return item*1.08")`. 16 built-ins + custom. 65 tests. Zero deps.
- **50 tools total**: memory, search, code, fetch, browser, email, file, voice, rss, scheduler, database, git, table, webhook, http, cache, notify, json, datetime, process, env, crypto, validator, metrics, template, diff, retry, html, xml, regex, rate_limit, queue, event_bus, state_machine, map_reduce, graph, format, search_index, config, chunker, vector_store, timer, stats, sampler, workflow, alert, lock, audit, batch, and custom via `@tool`.
- **3 providers**: Anthropic, OpenAI, OpenRouter free tier.
- **2328 tests.** (391 when this article was drafted; thirty more versions shipped before publication.)
