#!/usr/bin/env python3
"""
Re-grade all MCP servers in leaderboard.html with agent-friend v0.85.0.
Check 35: description_redundant_type — param description starts with its type name.
"""
...
MANUAL_MAP = {
    ...
    'ea-playwright':        f'{RESEARCH}/playwright-mcp-tools.json',
...
def get_redundant_type_details(schema_path):
    """Get details about description_redundant_type issues in a server."""
    try:
        data = json.load(open(schema_path))
    except:
        return []
    ...
    try:
        issues, _ = validate_tools(data)
        return [i for i in issues if i.check == 'description_redundant_type']
    except:
        return []
    ...
    def replace_detail(m):
        ...
        if len(bars) >= 3:
            ...
            new_e = (f'<div class="breakdown-bar-bg"><div class="breakdown-bar-fill" data-width="{e_score}" '
                     ...
                     f'<span class="breakdown-score" style="color: {e_color};">{e_score}</span>')
            new_q = (f'<div class="breakdown-bar-bg"><div class="breakdown-bar-fill" data-width="{q_score}" '
                     f'style="background: {q_color};"></div></div>\n                    '
                     f'<span class="breakdown-score" style="color: {q_color};">{q_score}</span>')
...
def main():
    ...
    for server_id in server_ids:
        ...
        rt_issues = get_redundant_type_details(schema_path)
        if rt_issues:
            all_rt_issues.append((server_id, rt_issues))
    ...
    if all_rt_issues:
        print(f"\n=== description_redundant_type issues ({len(all_rt_issues)} servers) ===")
        for server_id, issues in sorted(all_rt_issues, key=lambda x: -len(x[1])):
            print(f"  {server_id}: {len(issues)} issues")
            for iss in issues[:3]:
